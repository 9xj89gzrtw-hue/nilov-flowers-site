...
function seedDatabase(PDO $pdo): void
{
    ...
    /* W98-fixE (E13): 8 зон вместо демо-3 — витрина обещает доставку «по районам
       Санкт-Петербурга», селект формы и zone-check должны ей соответствовать */
    $pdo->exec("INSERT INTO delivery_zones (name, price, sort) VALUES
        ('Центральный', 300, 10), ('Северный', 400, 20), ('Южный', 400, 30),
        ('Петроградский', 350, 40), ('Василеостровский', 400, 50),
        ('Московский', 450, 60), ('Невский', 450, 70), ('Приморский', 500, 80)");
...
function migrateSchema(PDO $pdo): void
{
    ...
    $pdo->exec("INSERT OR IGNORE INTO settings (key, value) VALUES
        ...
        ('seo_text_body', 'Доставляем букеты по районам Санкт-Петербурга в день заказа — от центра до удалённых кварталов. Оформите заказ до 20:00, и курьер привезёт цветы сегодня; точное время согласуем по телефону. Для срочных случаев собираем букет за 1–2 часа.
...
Способ оплаты выберете при оформлении: наличными курьеру при получении или онлайн — если доступен в заказе. Стоимость доставки зависит от района — от 300 ₽ по центру; точную сумму посчитаем при подтверждении заказа.'),
        ...
        ('faq_a1', 'Зависит от района: 300–500 ₽ по СПб, самовывоз бесплатный. Точная сумма сразу видна при оформлении заказа.'),
    ...
    $pdo->exec("INSERT OR IGNORE INTO settings (key, value) VALUES
        ('category_intro_rozy', 'Свежие розы с утренней поставки — от компактных монобукетов до 25 роз в премиальной упаковке. Соберём и доставим в день заказа по СПб, оплата при получении.'),
        ...
        ('category_intro_polevye-cvety', 'Букеты из полевых и садовых цветов — лёгкие, ароматные, как из деревенского сада. Доставка по СПб в день заказа, оплата при получении.'),
        ('category_intro_avtorskie-bukety', 'Авторские композиции нашего флориста — собираем утром из свежей поставки, фото букета пришлём до отправки. Доставка по СПб в день заказа.'),
